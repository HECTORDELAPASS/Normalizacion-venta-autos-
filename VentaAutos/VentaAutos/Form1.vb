...
Public Class Form1
    ...
    Public Sub New()
        ...
        query = "select id, nombreestado from estado order by nombre"
    ...
    Private Sub CboEdo_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CboEdo.SelectedIndexChanged
        If auto Then
            Exit Sub
        End If
        query = "SELECT * FROM buscaedo WHERE idestado = " & CboEdo.SelectedValue
    ...
    Private Sub CboAgen_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CboAgen.SelectedIndexChanged
        If CboEdo.Text Is Nothing Then
            Exit Sub
        End If
        query = "select * from BusquedaCarro where [Nombre Agencia] = '" & CboAgen.Text & "' and [Estado De Venta] = '" & CboEdo.Text & "'"
    ...
    Private Sub filtrar()
        ...
        If filtradoFecha And R1.Checked Then
            query = "select * from BusquedaCarro where [Nombre De carro] like '%" & txtbuscar.Text & "%' and [Estado De Venta]  = '" & CboEdo.Text & "' and [Año de carro] BETWEEN " & primero.ToString & " And " & segundo.ToString & " order by [Año de carro] asc"
            DataGridView1.DataSource = Connection.SelectQuery(query)
            '''''''''''''''''''
            actualizayear()
        ...
        Else
            If txtbuscar.Text = "" Then
                Exit Sub
            End If
            query = "select * from BusquedaCarro where [Nombre De carro] like '%" & txtbuscar.Text & "%' and [Estado De Venta]  = '" & CboEdo.Text & "'"
            DataGridView1.DataSource = Connection.SelectQuery(query)
            '''''''''''''''''''
            actualizayear()
        End If
    ...
    Private Sub actualizayear()
        Try
            query = "select min([Año de carro]) as [AñoMinimo], max([Año de carro]) as [AñoMaximo] from BusquedaCarro where [Nombre De carro] like '%" & txtbuscar.Text & "%' and [Estado De Venta]  = '" & CboEdo.Text & "'"
            Dim dt As DataTable
